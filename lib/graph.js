...
function computePageRank(notes, outgoingEdges, incomingEdges, iterations = 30, damping = 0.85) {
  ...
  for (let step = 0; step < iterations; step += 1) {
    ...
    for (const id of ids) {
      ...
      for (const sourceId of incomingEdges.get(id) ?? []) {
        ...
        // Use explicit centrality if present
        const note = notes.find(n => n.id === sourceId);
        const sourceWeight = (note && typeof note.explicitCentrality === 'number') 
          ? Math.max(0.1, note.explicitCentrality) 
          : 1.0;

        const contribution = outgoing.length > 0 ? (sourceRank * sourceWeight) / outgoing.length : sourceRank / total;
